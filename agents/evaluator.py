...
def router_node(state: EvaluatorState):
    ...
    if tasks and state.get("followup_cycles", 0) < 1:
        return [
            Send(
                "scout",
                {"AssignedTask": ResearchTask(topic=t.topic, additional_info=t.additional_info)},
            )
            for t in tasks
        ]
    return "end"
...
def llm_node(state: EvaluatorState) -> EvaluatorState:
    ...
    system_prompt = EVALUATOR_PROMPT.format(interests=config.interests, user_profile=user_background)
    ...
    if len(state.get("InvestigatorTasks", [])) > 0:
        messages.append(HumanMessage(content=json.dumps({"InvestigatorTasks already completed": [t.model_dump() for t in state["InvestigatorTasks"]]})))
    response = structured_llm.invoke(messages)
    return {"NewsStories": response.NewsStories, "InvestigatorTasks": response.InvestigatorTasks}
...
def clear_tasks_node(state: EvaluatorState) -> EvaluatorState:
    ...
    preview = [{"title": s.title, "url": s.url} for s in stories[-3:]] if stories else []
