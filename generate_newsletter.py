...
def generate_scout_tasks(state: PlannerState):
    ...
    return [Send("scout", {"AssignedTask": task, "config": state["config"]}) for task in state["ScoutTasks"]]
...
Examples:
  ...
  %(prog)s --config custom.yaml     # Use custom configuration file
    ...
    try:
        ...
        print(f"     - {', '.join(config.interests[:3])}{'...' if len(config.interests) > 3 else ''}")
        ...
        initial_state: dict = {"config": config, "ScoutTasks": [], "NewsStories": [], "Report": ""}
