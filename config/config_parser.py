...
class RedditSettings(BaseModel):
    ...
    min_upvotes: int = Field(
        default=100, ge=0, description="Minimum upvotes for consideration"
    )
    max_age_hours: int = Field(
        default=48, gt=0, description="Maximum age in hours for posts"
    )
...
class YouTubeSettings(BaseModel):
    ...
    min_views: int = Field(
        default=10000, ge=0, description="Minimum views for consideration"
    )
    max_duration_minutes: int = Field(
        default=60, gt=0, description="Maximum video duration in minutes"
    )
...
class NewsletterSettings(BaseModel):
    ...
    tone: Literal["professional", "witty", "casual", "academic"] = Field(
        default="witty", description="Newsletter tone"
    )
    include_opinions: bool = Field(
        default=True, description="Include AI-generated opinions"
    )
    frequency: Literal["daily", "weekly"] = Field(
        default="weekly", description="Newsletter frequency"
    )
    max_stories: int = Field(
        default=10, gt=0, le=50, description="Maximum stories per newsletter"
    )
...
class OutputSettings(BaseModel):
    ...
    include_summary_stats: bool = Field(
        default=True, description="Include reading statistics"
    )
...
class ThinkleConfig(BaseModel):
    ...
    interests: List[str] = Field(min_length=1, description="List of topics to follow")
    user_profile: str = Field(
        default="", description="Short background on the user to tailor content"
    )
    ...
    max_tasks: int = Field(
        default=1, gt=0, le=10, description="Maximum number of tasks to generate"
    )
...
class ConfigParser:
    ...
    def save_config(
        self, config: ThinkleConfig, output_path: Optional[Union[str, Path]] = None
    ) -> None:
        """
        ...
        with open(output_path, "w", encoding="utf-8") as file:
            yaml.dump(
                config_dict, file, default_flow_style=False, sort_keys=False, indent=2
            )
